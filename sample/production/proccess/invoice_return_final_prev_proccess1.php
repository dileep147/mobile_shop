...
if (isset($_POST["finalize"])) {
    ...
    try {
        save_returns($_SESSION["product_return"]);
...
function save_returns($sess_product_return) {
    ...
    // $qty = $invoice_data->balance;    
    // $qty = $qty -  $_SESSION['return_invoice_total'];    
    // $invoice_data->balance = $qty;    
    // $invoice_data -> save();
    ...
    $product_return->date_time = date("Y-m-d h:i:s");
    ...
    try {
        ...
        foreach ($product_return_batches as $sess_product_return_batch) {
            ...
            $data = Inventory::find_by_batch_id($return_batch_id);
            $invn_id = $data->id;
            $reqty = (-1)*$return_qty;
            $reamount = (-1)*$return_unit_price*$return_qty;

            $inv_inventory = new InvoiceInventory();
            $inv_inventory->invoice_id = $invoice_id;
            $inv_inventory->inventory_id = $invn_id;
            $inv_inventory->qty = $reqty;
            $inv_inventory->price = $return_unit_price;
            $inv_inventory->unit_discount = 0;
            $inv_inventory->gross_amount = $reamount;            
            $inv_inventory->net_amount = $reamount;
            $inv_inventory->save();

            // end of insert to invoice
            
            // update invoice

            $datan = Invoice::find_by_id($invoice_id);
            $gross = $datan->gross_amount;
            $net = $datan->net_amount;
            $balance = $datan->balance;

            $gross = $gross + $reamount;
            $net = $net + $reamount;
            $ball = $balance + $reamount;

            $datan->gross_amount = $gross;
            $datan->net_amount = $net;
            $datan->balance = $ball;
