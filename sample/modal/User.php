...
//    }
    ...
    public static function find_all_active(){
        ...
        $object_array=self::find_by_sql("SELECT name FROM ".self::$table_name." WHERE soft_delete = 0 ORDER BY id ASC");
        return array_shift($object_array);
    }
    ...
    public static function find_by_username($value){
        global $database;
        $value=$database->escape_value($value);
        $object_array=self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE username='$value' AND soft_delete = 0 ");
        return array_shift($object_array);
    }
    ...
    public static function find_all_by_designation_id($value){
        global $database;
        $value=$database->escape_value($value);
        $object_array=self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE designation_id='$value' AND soft_delete = 0 ");
        return $object_array;
    }
    ...
    public static function find_name_by_id($value){
        global $database;
        $value=$database->escape_value($value);
        $object_array=self::find_by_sql("SELECT name FROM ".self::$table_name." WHERE id='$value' AND soft_delete = 0 ");
        return array_shift($object_array);
    }
}
