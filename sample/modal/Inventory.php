...
class Inventory extends DatabaseObject {
  ...
  public function product_id() {
    return $this->batch_id()->product_id();
  }
  ...
  public static function find_all_by_barcode($value) {
    global $database;
    $value = $database->escape_value($value);
    $object_array = self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE barcode = '$value' ");
    return $object_array;
  }

  public static function find_all_by_barcode_last($value) {
    global $database;
    $value = $database->escape_value($value);
    $object_array = self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE barcode = '$value' ORDER BY id DESC LIMIT 1 ");
  ...
  public static function find_by_grn_id($value) {
    global $database;
    $value = $database->escape_value($value);
    $object_array = self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE reference_id='$value' AND type='GRN' ");
    return $object_array;
  }
